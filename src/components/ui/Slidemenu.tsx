...
const sideStyle = {
    ...
    bottom: "inset-y-0 left-0 h-full w-3/4 border-r sm:max-w-sm",
    left: "inset-x-0 bottom-0 h-auto border-t"
}
    ...
    return (
        <Portal data-slot="Slide-menu-portal">
            <Overlay 
                ...
                className="fixed inset-0 z-50 bg-foreground/10 backdrop-blur-xs"
            >
                <Content 
                    data-slot="Slide-menu-content"
                    className={`fixed z-50 flex flex-col shadow-lg h-full w-64 bg-popover text-popover-foreground ${sideStyle[side]} ${className}`}
                    {...rest}
                >
                    {children}
                </Content>
