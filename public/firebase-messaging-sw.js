...
self.addEventListener('push', (event) => {
    ...
    if( event.data ){
        ...
        const promesaAvisarSO = self.registration.showNotification(data.title, { 
            body: data.body,
            data:  {
                url: data.url
            }
        });
        const promesaAvisarApp = clients.matchAll({ type: "window", includeUncontrolled: true })
                                    .then((clientList) => {
        ...
        const promesas = Promise.all([promesaAvisarSO, promesaAvisarApp]);
        event.waitUntil(promesas);
    };
});
